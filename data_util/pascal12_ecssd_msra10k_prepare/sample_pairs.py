'''
 ...
 Note that each image/mask pair only contains single object in pascal12_ecssd_msra10k dataset. There are 10671 objects,
 for each object there are 50 augmented pairs. By default, for each object, we only sample 5 pairs, idealy resulting to
 5 * 10671 = 53355 training pairs

 Sampling procedure:
    for each obj_dir:
        pick 5 pairs randomly, check/set obj_id=1

 Actually sampled pairs: 53354 (one pair is broken and skipped)
'''
...
def main():
    ...
    if len(obj_dirs) != NUM_OBJS:
        print('Num of obj_dirs {} != num of objects {}'.format(len(obj_dirs), NUM_OBJS))

    # sampling
    sample_list = []
    count = 0
    print('Starting sampling for {} objects'.format(len(obj_dirs)))
    for obj_dir in obj_dirs:
        ...
        for i in range(5):
            ...
            # skip invalid object ids and segmentations
            if not np.array_equal(seg_idsL, seg_idsR):
                print('seg_idsL {} != seg_idsR {} for {}'.format(seg_idsL, seg_idsR, re_segsL[i]))
                continue
            if len(seg_idsL) != 2 or (not 0 in seg_idsL) or (not 1 in seg_idsL):
                print('Seg labels {} not valid for {}'.format(seg_idsL, re_segsL[i]))
                continue
            # img1_dir, gt1_dir, img2_dir, gt2_dir, obj_id
