'''
The files generates COCO17 train tfrecords.
'''
...
_NUM_PAIRS = 141187  # valid number of pairs is: 141187
_NUM_SHARDS = 128  #
_PAIRS_PER_FILE = 1103  # 1103 x 127 = 140081
_PAIRS_LAST_FILE = 1106  # 1106 x 1 = 1106
...
def generate_train_shard(pairs_list, out_dir, num_examples, shard_id, coder):
    ...
    for pair_line in pairs_list:
        ...
        # write to file
        example = convert_to_example(image_buffer0, image_buffer1, encoded_anno0, encoded_anno1, object_id, height0,
                                     width0)
...
def generate_train_shards_process(pairs_list, out_dir, shards_per_proc, _PAIRS_PER_FILE, _PAIRS_LAST_FILE, is_last_proc,
                                  proc_id):
...
def main(args):
    ...
    print('Generate COCO17 Training tfrecords.')
    ...
    pairs_list = get_pairs_list(source_dir)  # should be 141187 pairs
    shards_per_proc = int(num_shards / num_proc)  # should be 128/8=16 by default
    # use GPUs
    ...
    for proc_id in range(num_proc):
        ...
        process_pool.append(multiprocessing.Process(target=generate_train_shards_process,
                                                    args=[pairs_list[start:end], out_dir, shards_per_proc,
                                                          _PAIRS_PER_FILE,
...
if __name__ == '__main__':
    ...
    arg_parser.add_argument('--source_dir', dest='source_dir',
                            default='/storage/slurm/wangyu/coco/')
    arg_parser.add_argument('--out-dir', dest='out_dir', default='/storage/slurm/wangyu/coco/tfrecord_train/')
